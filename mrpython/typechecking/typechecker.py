...
class TypingContext:
    def __init__(self, prog):
        ...
        #0: cannot be modified nor be copied ;1: cannotbe modified but can be copied; 2: right for modification and copy
        self.modifiable_vars={}
        #not a fan of putting those kind of things in the context, TODO: think about a more proper solution
        self.encoutered_argument = False
        self.is_modifiable = 0
    ...
    def register_parameters(self, parameters, param_types):
        ...
        for (param, param_type) in zip(parameters, param_types):
            ...
            print("type du parametre : " + str(param_type))
            if isinstance(param_type, ListType):
                print("hello hello")
                self.modifiable_vars[param] = 1
...
def check_assign_rules_list (expr):
    for elem in expr.elements:
        elem.check_assign_rules()

def check_assign_rules_var (expr):
    print("var vaut " + str(expr))

EList.check_assign_rules = check_assign_rules_list
EVar.check_assign_rules = check_assign_rules_var
...
def type_check_Assign(assign, ctx, global_scope = False):
    ...
    for var in assign.target.variables():
        ...
        #e check that the variable is not "dead"  (out of scope)
    ...
    #print("var vaut " + str(assign.expr))
    if mono_assign:
    ...
    if assign.target.arity() == 1:
        ...
        #(assign.expr).check_assign_rules()
        print("var : " + str(assign.expr))
...
def type_infer_EAdd(expr, ctx):
    ...
    prv_ctx_is_modifiable = ctx.is_modifiable
    ctx.encountered_argument = False;
...
def type_infer_EVar(var, ctx):
    ...
    print("var name : " + str(var.name) + " val dic " + str(ctx.modifiable_vars[var.name]))
    
    if ctx.modifiable_vars[var.name] == 1:
        ctx.encountered_argument = True
        ctx.is_modifiable=1
    
    if ctx.modifiable_vars[var.name] == 0:
        ctx.is_modifiable=0
    if ctx.param_env and var.name in ctx.param_env:
        
        #check if param is a mutable type, if so flag the variable
        tmp = ctx.param_env[var.name]
        print("hello" + str(type(tmp)))
        return tmp
...
#Function was checked before and is a potentially side effect function
"""
def receiver_in_parameters (call, ctx):
    if(ctx.param_env.get(call.receiver.name) != None):
        return True
    return False
"""
...
def type_infer_ECall(call, ctx):
    ...
    if call.full_fun_name in ctx.global_env:
        ...
        #print("receveur : " + str(call.receiver.name))
    ...
    num_arg = 1
    #if cpt=1, we are checking the receiver
    cpt = 1
    ...
    for (arg, param_type) in zip(arguments, signature.param_types):
        ...
        print("arg vaut " + str(arg))
        #pas de side affect append
        if ctx.is_modifiable == 0 or ctx.is_modifiable == 1 and cpt == 1 and method_call:
            print("POTENTIALLY SIDE EFFECT CALL : ")
...
class SideEffectFunctionError(TypeError):
    def __init__(self, in_function, call):
        self.in_function = in_function
        self.call = call
    ...
    def fail_string(self):
        return "SideEffectFunctionError[{}]@{}:{}".format(self.call.full_fun_name, self.call.ast.lineno, self.call.ast.col_offset)

    def report(self, report):
        report.add_convention_error('error', tr("Call problem"), self.call.ast.lineno, self.call.ast.col_offset
                                    , tr("{}' should not be applied to a parameter").format(self.call.fun_name))
